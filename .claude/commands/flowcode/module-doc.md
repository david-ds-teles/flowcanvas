...
links: [.claude/skills/flowcode/module-doc/SKILL.md, .claude/agents/flowcode/module-explorer-agent.md, .flowcode/project/modules/README.md, .claude/commands/flowcode/bootstrap.md]
---

# /flowcode:module-doc

- Thin entry point: loads and runs the shared `flowcode:module-doc` skill at `.claude/skills/flowcode/module-doc/SKILL.md` — the procedure lives in the skill (which dispatches `flowcode:module-explorer-agent`), not here.
...
Load `.claude/skills/flowcode/module-doc/SKILL.md` and execute its procedure. Resolve the requested module(s) — `$ARGUMENTS` if provided, otherwise list missing/stale docs and ask — then dispatch `flowcode:module-explorer-agent` (model **sonnet**) per module in merge-mode and relay the reports.
