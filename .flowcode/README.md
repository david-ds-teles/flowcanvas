...
## Testing Changes with mdview

`mdview/` is the bundled host project used to evaluate flowcode in real conditions. Instead of reinstalling on every edit, `link_project.js` symlinks the source tree directly into `mdview/` so changes take effect immediately — no reinstall, no copy step.
...
```bash
node flowcode/link_project.js mdview
```

Framework files in `.flowcode/` and `.claude/` become symlinks pointing at the live source. Edits to `flowcode/` are visible to Claude Code in `mdview/` instantly.
...
```bash
node flowcode/link_project.js mdview --migrate
```
...
```bash
node flowcode/link_project.js --unlink mdview
```
...
```bash
node flowcode/link_project.js --unlink --purge-artifacts mdview
```
