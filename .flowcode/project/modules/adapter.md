...
// edges[0].markerEnd         deepEquals { type: MarkerType.ArrowClosed }
...
**Upstream modules:**
- `lib/canvas/jsoncanvas` — all domain types (`FlowcanvasDoc`, `CanvasNode`, `CanvasEdge`, `CanvasColor`, `Side`, `EdgeOrigin`, `RelationshipType`) + `nodeKind` runtime discriminator + `COMPONENT_KIND_META` (per-kind accent table used for boundary-group `--node-accent` injection; `adapter.ts:4`)

**External services:**
- None.

**Key libraries:**
- `@xyflow/react` — `MarkerType` runtime enum (the sole non-type import in `lib/canvas/*`; `adapter.ts:1`); `RFNode` and `RFEdge` are type-only imports
...
**Conventions & patterns:** The adapter is the React Flow translation boundary — the only file in `lib/canvas/` that imports a non-type runtime symbol from `@xyflow/react` (`MarkerType` at `adapter.ts:1`). All other `lib/canvas/*` modules import zero React or RF symbols. The boundary is enforced by convention and the vitest gate (no browser environment): the adapter stays DOM-free so tests run without jsdom. `nodeKind` (from `jsoncanvas.ts`) is called once per node (`adapter.ts:15`) to derive `kind`. `renderType` is then computed at `adapter.ts:18`: if the node carries `meta.kind` and is not a `type:'group'` node, `renderType` is `'component'` (routing to `ComponentNode`); otherwise `renderType === kind`. The RF `type` field is set to `renderType` (`adapter.ts:30`), so React Flow dispatches to the correct renderer at runtime.
...
- **`toJSONCanvas` carries RF-invisible edge fields from `prev`.** React Flow's edge model does not carry `color`, `fromEnd`, or `toEnd`. `toJSONCanvas` spreads `...base` from the snapshot at `adapter.ts:71` to preserve these. If `prev` is stale relative to the incoming RF edges (e.g. a newly-added edge has no `base`), those fields will be absent in the output — callers must keep `prev` current.
