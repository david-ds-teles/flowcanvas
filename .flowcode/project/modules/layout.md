...
- Public API: `computeLayout(nodes, edges, measured?) → Promise<LayoutPositions>`; exported types `LayoutPositions` and `MeasuredSizes`.
- Pure async module — no DOM, no React, no filesystem. Only consumer: `components/canvas/canvas-toolbar.tsx`.
