---
name: store
description: Module knowledge base for Store — the Zustand useCanvasStore that owns all canvas state and actions; the most-depended-on module in Flowcanvas.
status: active
...
- Public API: `useCanvasStore` store + `CanvasMode` + `ReaderSize` exported types; 40+ actions grouped by lifecycle: load/save, node editing, edge editing, comments, reader, selection/group/layout, v2 agent round-trip, v2 navigation/templates/reconcile, Phase 5 board import.
...
```ts
// lib/canvas/store.ts:144
load(path: string): Promise<void>
// Canvas-authoritative load. Routes through migrateDoc (0.1→0.2→0.3 shared ladder); if migrated,
// calls save() once to persist schemaVersion:'0.3' (store.ts:159). Writes active-board pointer;
...
// seeds coreDocBody/Draft; resets all five Phase 4 transient fields on every load (store.ts:155-158).
...
// lib/canvas/store.ts:313
newBoard(): Promise<void>
// Create and adopt a fresh empty `untitled-*.canvas` board with schemaVersion: '0.3'; resets all
// transient state including the five Phase 4 fields (coreDocBody/Draft/Dirty/spineHighlightAnchor/linkedNodeIds) (store.ts:324-325).
...
// If target node already on board: focus it. Else: add it near source + draw rel:'references' edge.
...
```ts
// lib/canvas/store.test.ts:25-37 — seed the store outside React and exercise onConnect
...
// 2. Draw a connection — mints a typed user edge (rel:'related') + opens inline editor
...
// minted -> { fromNode:'b', toNode:'a', fromSide:'right', toSide:'left',
//             label:'', toEnd:'arrow', meta:{ origin:'user', rel:'related' } }
// editingEdgeId === minted.id   (inline label editor opens immediately)
// dirty === true
```

Real call site: `lib/canvas/store.test.ts:25-37`. Demonstrates: Zustand store seeded outside React, `onConnect` minting a v2 typed user edge with `rel:'related'`, and the inline-editor signal.
...
- `lib/canvas/brief.ts` — `buildBriefPure`, `applyResponsePure`, `AgentResponse`, `DesignBrief`, `MergeReport` (store.ts:6-7)
- `lib/canvas/review.ts` — `diffDocs`, `ReviewState`, `ReviewDiff` (store.ts:8-9)
- `lib/canvas/templates.ts` — `instantiateTemplate`, `CanvasTemplate` (store.ts:13-14)
- `lib/canvas/spine.ts` — `buildSourceIndex` (resolves anchor → node-id list in `highlightComponents`; spine.ts:37) + `normPath` (shared path-normalisation helper used in `navigateRef` and `resyncFile`; the local `normPath` previously inlined in the store was removed when this import was added in Phase 4) (store.ts:10)
- `lib/canvas/refs.ts` — `DocRef` type-only import for `navigateRef` parameter (store.ts:15)
- `lib/api.ts` — all HTTP wrappers: `getCanvas`, `saveCanvas`, `resolvePaths`, `writeFileApi`, `readFileApi`, `deleteFileApi`, `getReview`, `putReview`, `clearReview`, `putActive` (store.ts:16; `readFileApi` added in Phase 4 for core-doc body resolution in `load` and `setCoreDoc`)
