...
## [BUGFIX] Edge arrowheads hidden until selected · hand-tool drag bent edges instead of panning — 2026-07-01

**Dev:** david-ds-teles <david.ds.teles@gmail.com>
**Cause:** Two operator-reported edge defects. (1) **Arrowheads only showed when the edge was clicked/selected.** Verified via CDP that the marker is referenced (`marker-end=url(#…)`) AND its `<marker>` def exists in the DOM at rest — so not conditional logic. `elementFromPoint` at the endpoint returned `DIV.react-flow__handle`: the always-visible connection **port dot** (`.fc-port::before`, an OPAQUE `background: var(--color-surface-highest)` disc in the nodes/handles layer) sits directly over the arrowhead, which paints in the edges layer BELOW it. React Flow elevates an edge above the nodes layer only on selection (`elevateEdgesOnSelect`) — the one time the head cleared the dot. (2) **The new hand/pan tool bent edges instead of panning:** the edge's fat `.fc-edge-grab` hit-path (+ RF's `.react-flow__edge-interaction`/visible `.react-flow__edge-path`, all pointer-interactive) captured a drag over a line in pan mode — inserting a waypoint and eating the pan — because the pan-mode neutralisation only covered the node connection dots, not the edge surfaces.
**Fix:** `edges.css`. (1) The port dot `::before` centre is now `background: transparent` (a ring, not a filled disc), so the arrowhead — anchored at the same port point — shows THROUGH the dot at rest ("head seats in the dot", now visibly). (2) Extended the `.fc-rf--pan` neutralisation to every edge surface — `.react-flow.fc-rf--pan .react-flow__edge path` (covers the visible path, RF's interaction path, and `.fc-edge-grab`) plus `.fc-edge-waypoint`/`.fc-edge-label`, all `pointer-events: none !important` — so a hand-tool drag over an edge falls through to the pane and pans (no bend). Scoped to pan mode only; normal-mode edge bending / dot-grabbing is unchanged. No schema/agent-contract change.
**Affected:** `app/styles/edges.css`. Gates: tsc 0 · lint 0 · vitest 259/259 · build ok · live CDP verify PASS on `boards/msgflow-mvp-design.canvas` — (1) tight-crop screenshot of a `data-flow` edge shows the cyan arrowhead rendered at rest, seated in the ring dot (DOM already proved marker referenced+defined at rest); (2) in pan mode a drag starting exactly on an edge line is INERT (elementFromPoint ≠ edge surface), PANS the viewport, and leaves the edge path `d` unchanged (no bend); pan-toggle/Shift+3 suite still PASS. Commit held — operator controls commits (working tree carries open 005/006/007 changes).

---

## [QUICKFIX] Big-board navigation — pan tool (Space-hold + H toggle) · Figma-style fit/context shortcuts — 2026-06-30

**Dev:** david-ds-teles <david.ds.teles@gmail.com>
**Related:** standalone (operator: holding Space must navigate the board even over a component; holding it constantly is tiring so pan needs a toggle; a large design like `boards/msgflow-mvp.canvas` needs a fast way to absorb the whole thing AND to see a component in the context of its connections; [[verify-interactions-end-to-end]]). Iterated once on operator feedback (added the pan toggle + Shift+3).
**What:** (1) **Pan (hand) tool** — a drag anywhere (over a component, group, empty canvas) PANS the board instead of grabbing the node under the cursor. Two ways in: **hold Space** (transient) OR the new **hand tool** in the toolbar mode group / **`H`** key (sticky toggle — no need to hold). (2) **Framing shortcuts** — `Shift+1` fit the whole board · `Shift+2` zoom TIGHT to the selection · `Shift+3` a context step-back that centres on the selection at **half its tight (Shift+2) zoom** so the component stays prominent with a ring of surroundings · `Shift+0` reset to 100%. (3) All gestures + shortcuts documented in `README.md`.
**How:** `store.ts` — `CanvasMode += 'pan'` (sticky hand tool). `canvas-toolbar.tsx` — a hand-tool `modeBtn('pan', …)` in the mode group. `canvas-shell.tsx` — `panActive = spacePan || mode === 'pan'`; while active the `<ReactFlow>` switches to pan-anywhere (`nodesDraggable`/`elementsSelectable`/`selectionOnDrag` off, `panOnDrag` on) and takes the `fc-rf--pan` class; a window keydown effect drives Space-hold + the `H` toggle (Space yields to a focused button so it doesn't hijack activation, `H` only blocked inside text fields). `edges.css` — `.fc-rf--pan` sets the always-visible connection dots (`.fc-port`/`.fc-port-add`) to `pointer-events:none`, so the 007 capture-phase dot-gesture (which bails on `target.closest('.fc-port')`) stands down and the drag reaches React Flow's pan; the surface reads grab → grabbing. Framing shortcuts use `useReactFlow().fitView`/`zoomTo`/`setCenter`/`getInternalNode` in a keydown effect (`e.code` so keyboard-layout glyphs don't matter); Shift+3 computes the selection's absolute bounds and `setCenter`s at half the tight-fit zoom (component-centric context — no fit-all blow-out, the first neighbour-union pass zoomed out too far). No schema/agent-contract change.
**Affected:** `lib/canvas/store.ts` · `components/canvas/canvas-shell.tsx` · `components/canvas/canvas-toolbar.tsx` · `app/styles/edges.css` · `README.md`. Gates: tsc 0 · lint 0 · vitest 259/259 · build ok · live CDP verify PASS on `boards/msgflow-mvp-design.canvas` — framing: Shift+0→100% · Shift+1 fit-all 1→0.208 · Shift+2 tight 0.208→1.76 · Shift+3 context 1.76→0.73 (≈half the tight zoom, hub `ag-zavu` stays centred dx/dy≈0, well zoomed-in vs fit-all 0.21 — no blow-out); pan: Space+drag AND hand-tool-drag both panned the viewport while the node's flow transform stayed identical · hand tool toggles on click, `H`, and off via the arrow tool. Commit held — operator controls commits (working tree carries open 005/006/007 changes).

---

## [BUGFIX] Canvas zoom vs card scroll — gesture-aware wheel routing over component cards — 2026-06-30

**Dev:** david-ds-teles <david.ds.teles@gmail.com>
**Cause:** Operator-reported: the wheel could not zoom in/out whenever the cursor sat on a component card with text. The prior `[QUICKFIX] Agent board generation` put the React Flow `nowheel` class on `.fc-cmp__body` so the inline spec body could wheel-scroll — but `nowheel` is an all-or-nothing filter that makes React Flow skip ALL wheel handling (zoom AND pinch) for any wheel over that element. Since the inline body fills most of a card and generated boards are almost entirely component widgets, the cursor landed on a wheel-trapping body nearly everywhere → zoom appeared dead. (Markdown/note nodes use `overflow:hidden`, no `nowheel`, so only the component widget was affected.) Operator refinement: zoom and scroll are DIFFERENT gestures and both should work depending on the gesture — not one-or-the-other.
**Fix:** Replaced `nowheel` with a gesture-aware native `wheel` listener on the body (`component-node.tsx`): a plain wheel scrolls the card while it still has content to scroll; **pinch / ⌘+wheel always zooms the canvas** (the listener returns, letting the event reach React Flow); and once the body is scrolled to its edge the wheel falls through to canvas zoom (no trap). The listener runs in the bubble phase, so `stopPropagation` beats React Flow's zoom handler on the ancestor pane only when the card actually consumes the scroll. Updated the `.fc-cmp__body` CSS comment (`nodes.css`). No schema/agent-contract change.
**Affected:** `components/canvas/nodes/component-node.tsx` · `app/styles/nodes.css`. Gates: tsc 0 · lint 0 · vitest 259/259 · build ok · live CDP verify PASS on `boards/msgflow-mvp-design.canvas` (plain wheel over an overflowing card scrolled it scrollTop 0→480 with canvas scale unchanged · ctrl/pinch wheel over the same card zoomed 1.76→2). Operator flagged a follow-up redesign of the canvas component widgets — out of scope. Commit held — operator controls commits (working tree carries open 005/006/007 changes).

---

## [QUICKFIX] Connection UX — click-to-connect · drag the dot anywhere on the border · animated armed line · Esc-abort — 2026-06-30
...
**Root cause / gap:** 006 shipped connection ports, but the interaction was undiscoverable: connecting was a React-Flow handle DRAG, moving a dot was a hidden Alt-drag, dots were small + (after a first pass) hover-only, and there was no armed-connection feedback. Operator spec: connecting should be click-to-arm with an animated line; target dots should react on hover ("a connection is about to happen"); Esc should abort any in-progress operation (so the cursor line never gets stuck); and the connection dot should be a visible circle you can drag anywhere along the component border. Verified via live CDP that the move-the-dot code itself already worked (59px drag) — the real gap was discoverability of the dot + the connect gesture, not the geometry.
**Fix:** Reworked the canvas connect model into design-tool-grade click-to-connect, full agent parity preserved (NO schema change — `ports` + `edgeType` already in the contract). (1) `store.ts` — transient `connecting` state + `beginConnect`/`completeConnect`/`cancelConnect` (endpoint dots are minted only on LAND, so an aborted arm leaves no stray dot); `onConnect` refactored onto a shared `makeUserEdge`. (2) `use-canvas-handlers.ts` — one capture-phase pointer listener owns the gesture: TAP a dot/side handle = arm; TAP a target dot/node = land (node body → geometric `autoPort`); DRAG a real dot = slide it along the border (free by default, hold Shift to snap to side-centre/corners); global Esc = cancel — replaces 006's drag-to-connect + Alt-drag-to-move. (3) `connection-overlay.tsx` (NEW) — a `ViewportPortal` cursor-following marching-ants line + glowing endpoints + a screen-fixed "Esc to cancel" hint. (4) `port-handles.tsx` + `edges.css` — the connection dot is now ALWAYS visible and grabbable (22px transparent hit pad painting a 10px dot via `::before`, `cursor:grab/grabbing`); the 4 side "add" handles start a NEW connection (hover-reveal, dashed); target dots pulse cyan while armed. (5) `canvas-shell.tsx` — mounts the overlay + toggles the `fc-rf--connecting` class.
**Affected:** `lib/canvas/store.ts` · `components/canvas/{use-canvas-handlers,canvas-shell}.tsx` · `components/canvas/connection-overlay.tsx` (new) · `components/canvas/nodes/port-handles.tsx` · `app/styles/edges.css` · tests `lib/canvas/store.test.ts` (+6) · new `scripts/verify-connect.mjs` · research `.flowcode/researches/connection-endpoint-positioning-research.md`. Gates: tsc 0 · lint 0 · vitest 259/259 (+6) · build ok · smoke:mcp PASS · smoke:render PASS · live CDP verify PASS (A — drag-the-dot Δ59px · B — click-arm → animated line follows cursor → target-dot pulse → land creates edge · C — Esc aborts armed line, 0 edges). Module docs (`store`, `canvas-nodes`) sync pending. Commit held — operator controls commits (working tree also carries open 005/006 changes).

---

## [QUICKFIX] Agent board generation — readable wrapped layout + rich, structured component cards — 2026-06-30

**Dev:** david-ds-teles <david.ds.teles@gmail.com>
**Related:** standalone (operator-reported: generated boards are an unreadable horizontal strip AND the components are low-quality — "nothing concrete that helps a human understand the design").
**Root cause:** Two compounding defects in MCP/agent board generation. (1) `layout.organizeByType` placed every boundary group in a SINGLE left→right row at `y=80` and made each component-kind its own column, so a real board (`boards/msgflow-mvp-design.canvas`, 6 groups, agent group 1984px) rendered as an ~8000×600 horizontal strip. (2) The component widget (`component-node.tsx`) only ever rendered name + a one-line role + §source chip — the rich `.md` body was hidden behind a double-click — and the generation-kit EXTRACTION rule asked for "2–4 sentences", so cards were thin paragraphs (~300–560 chars) with no concrete contract. Spec-served ≠ spec-followed ([[mcp-spec-served-not-followed]]).
**Fix:** (A) `layout.ts` — rewrote `organizeByType` into a READABLE wrapped layout: children → balanced grid (band-ranked, bounded by `MAX_GROUP_WIDTH`), groups + loose leaves shelf-packed into tiered rows (sources → subsystems → notes) wrapping past `MAX_ROW_WIDTH`. (B) `component-node.tsx` + `nodes.css` — the widget now renders its full `.md` spec body INLINE (scrollable, kind-accented `**labels**` + bullets + `code`) so the component IS the detailed card. **Operator follow-up (same change):** the inline body had `nodrag`, so a press-drag on the body (now most of the card) moved nothing and fought the scroll — dropped `nodrag` (the card drags from anywhere; `nowheel` keeps wheel-scroll; `user-select:none` stops a text-selection fight), added a kind-tinted near-solid **header contrast band**, and raised the card fill opacity (.55 → .9) so the glass cards are legible. (C) `generation-kit.ts` — rewrote the EXTRACTION body rule + system prompt + RETURN CHECKLIST + worked example to require a STRUCTURED, CONCRETE spec (role + responsibilities + real contract: routes/signatures/tables/config/thresholds + constraints). (D) `brief.ts` `enforceBoardQuality` — new Audit 4 warns over MCP when a (re)generated node card is thin/unstructured, so the agent self-corrects ([[agent-feature-parity]]). (E) Regenerated `boards/msgflow-mvp-design` as proof — 25 cards enriched (~450 → ~1900 chars each, grounded in the core doc) + re-laid out (8000×600 strip → 2168×3254 portrait).
**Affected:** `lib/canvas/{layout,generation-kit,brief}.ts` · `components/canvas/nodes/component-node.tsx` · `app/styles/nodes.css` · `docs/flowcanvas-agent-contract.md` (regen) · tests `lib/canvas/{layout,brief,generation-pipeline}.test.ts` · `boards/msgflow-mvp-design.{canvas,nodes/*.md}` · module docs `{layout,generation-kit,brief,canvas-nodes}.md`. Gates: tsc 0 · lint 0 · vitest 253/253 (+4) · build ok · smoke:mcp PASS · smoke:render PASS · live CDP board verify PASS (25 rich widgets · avg 1657 inline body chars · 471 inline-code identifiers · 0.67:1 aspect · 0 overlaps / 0 children escaping) · live CDP drag verify PASS (inner component moves +(117,134)px from a body press-drag; header band bg alpha .94). Commit held — operator controls commits (working tree carries other open changes).
