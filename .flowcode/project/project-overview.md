...
| Store | `lib/canvas/store.ts` | Zustand store: `load`, `save`, `onConnect`, `toggleCollapsed`, `addComment`, `replyComment`, `resolveComment`, `buildBrief`, `applyResponse` orchestration | TypeScript/React | — pending (generated at phase close) |
...
| Edge component | `components/canvas/edges/labeled-edge.tsx` | Bezier edge with `EdgeLabelRenderer`; origin-styled: `links`=dashed+lock, `user`=solid, `agent`=accent-tinted | TypeScript/React | — pending (generated at phase close) |
