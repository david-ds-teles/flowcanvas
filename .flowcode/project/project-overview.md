...
- Status active; last updated 2026-06-27 — Phase 8 (post-execution polish) shipped CSS partials split, smoothstep edges, reader 3-size control, bidirectional link write-back, and 66/66 tests.
...
| Frontmatter | `lib/canvas/frontmatter.ts` | `gray-matter` parse wrapper; enforces `BODY_CAP` (40 000 chars); returns `{frontmatter, body, truncated}`; `stringifyFile(path, newFrontmatter)` merges updated frontmatter back into the file body-preservingly (scalars may re-quote via js-yaml) | TypeScript | — pending (generated at phase close) |
...
| Node components | `components/canvas/nodes/*` | `MarkdownNode` (frontmatter table + collapsible body + a `node-read` header button → reader drawer for the full doc), `ImageNode` (inline img via assetUrl), `LinkChipNode` (URL chip; `normalizeUrl` prepends `https://` to a scheme-less url so a bare `google.com` opens instead of 404ing), `NoteNode` (renders `text` markdown; **double-click → inline textarea edit** via `setNodeText`), `GroupNode` (a real shape, not a rectangle-with-an-svg: only the painted SVG outline — rectangle/ellipse/diamond per `meta.shape` — is hit-testable (`.fc-group` + the whole RF node wrapper are `pointer-events:none`, so corner clicks pass through), React Flow's rectangular `.selected` glow is suppressed and the shape stroke highlights instead; selecting shows a shape switcher (`setNodeShape`) + `NodeResizer` handles (`setNodeSize`); double-click label edit (`setNodeLabel`). The shell preserves RF `selected` across store re-syncs so editing doesn't deselect), `FallbackNode` (catch-all for non-md/non-image `file` kinds only). **Each returns a fragment** — the card `<div>` plus the four `<Handle>`s as *siblings* of the card (never nested inside the `overflow:hidden` card, or the right/bottom handles get occluded and connections only work from 2 of 4 sides) | TypeScript/React | — pending (generated at phase close) |
...
| Reader | `components/canvas/reader-drawer.tsx` (+ `lib/render-md.ts` server pipeline) | Full-fidelity read-only drawer; fetches `GET /api/render?path=` → sanitized shiki HTML (`remark`→`rehype-sanitize`→`@shikijs/rehype`); opens on a markdown-node click; shows the node's comment thread beneath the prose; 3-size segmented control (Drawer / Half / Full) drives `data-size` + CSS widths (440px / 50vw / 100vw) via `readerSize`/`setReaderSize`; `⤢` in the node header calls `maximizeReader` to jump to full size | TypeScript/React | — pending (generated at phase close) |
...
| API | `app/api/{canvas,canvas/resolve,canvas/links,asset,file,files,upload,render}/route.ts` | Eight guarded Node-runtime Route Handlers wrapping `fs`; all map `GuardError`→400, `ENOENT`→404, else→500. `canvas/links` (Phase 8): guarded `POST {path, op:'add'|'remove', link}` — body-preserving `links:` frontmatter patch via `frontmatter.stringifyFile`; surfaced as `patchLinks` typed wrapper in `lib/api.ts`. `render` (GET `?path=` → `{html}` full-fidelity shiki markdown) shipped Phase 7; all eight live | TypeScript/Next.js | — pending (generated at phase close) |
...
flowcanvas/
  app/                            — Next.js App Router root
    ...
    styles/                       — CSS partials @imported from globals.css (Phase 8 extraction)
      nodes.css                   — Node anatomy styles (.fc-node, .fc-prose, chips, collapse)
      ...
      reader.css                  — Reader drawer widths (440px/50vw/100vw per data-size)
      comments.css                — Comment pin + thread + beak styles
      toolbar.css                 — Toolbar, menus, dropzone, upload-error, mode-button
    api/
  ...
  components/
    canvas/
      ...
      use-canvas-handlers.ts      — RF controlled state + event callbacks + removeEdgeWriteback hook
