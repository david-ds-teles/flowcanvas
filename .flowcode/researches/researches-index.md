...
|------|-------|------|---------|------|
| `connection-endpoint-positioning` | Connection endpoint interaction mechanics — visibility, dragging, snapping | 2026-06-30 | Draggable dots on selected connections; snap-to-magnet by default (cardinal + center + corners); Ctrl/Cmd for free positioning. Cursor: grab on hover, grabbing when dragging. Hit target 24–32px. Figma/tldraw/Excalidraw all snap; React Flow uses static handles (no edge dragging). Recommended: visible on selection, cardinal+corner snapping, free-drag modifier, side+t model. | [.flowcode/researches/connection-endpoint-positioning-research.md](.flowcode/researches/connection-endpoint-positioning-research.md) |
