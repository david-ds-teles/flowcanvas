...
**Operations:**
- `GoogleClient.list_drive_pdfs(folder_id)` → `list[DriveFile]` — `files.list` filtered to `mimeType='application/pdf'` within the configured Drive folder (`knowledge_source.google_file_id`).
