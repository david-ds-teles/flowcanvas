...
4. **Text extraction:**
   - Primary: `pypdf` page-by-page.
   - Fallback to `pdfplumber` when `pypdf` text is empty OR printable-char ratio < 0.60.
   - Both empty → `document.ingest_status = 'failed'`.
5. Normalize whitespace; **tiktoken `cl100k_base`** token-chunk at `CHUNK_TOKENS=512` with `CHUNK_OVERLAP_TOKENS=64`.
6. Batch-embed (32 inputs/request) via `EmbeddingProvider → TEI sidecar POST /embed`.
...
- Embed question → ANN search: `SELECT … WHERE business_id=:bid ORDER BY embedding <=> :q LIMIT :k`.
